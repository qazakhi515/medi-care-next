...
const HospitalDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [destinationHospital, setDestinationHospitals] = useState<Hospital[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [hospitalComments, setHospitalComments] = useState<Comment[]>([]);
	const [commentTotal, setCommentTotal] = useState<number>(0);
	const [insertCommentData, setInsertCommentData] = useState<CommentInput>({
		commentGroup: CommentGroup.HOSPITAL,
		commentContent: '',
		commentRefId: '',
	});
	...
	const {
		...
		refetch: geetHospitalRefetch,
	} = useQuery(GET_HOSPITAL, {
		fetchPolicy: 'network-only',
		variables: { input: hospitalId },
		skip: !hospitalId,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getHospital) setHospital(data?.getHospital);
			if (data?.getHospital) setSlideImage(data?.getHospital?.hospitalImages[0]);
		},
	});
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: hospital?.hospitalLocation ? [hospital?.hospitalLocation] : [],
				},
			},
		},
		skip: !hospitalId && !hospital,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getHospitals?.list) setDestinationHospitals(data?.getHospitals?.list);
		},
	});
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getComments?.list) setHospitalComments(data?.getComments?.list);
	...
	useEffect(() => {
		...
		if (router.query.id) {
			setHospitalId(router.query.id as string);
			setCommentInquiry({
				...commentInquiry,
				search: {
	...
	const likeHospitalHandler = async (user: T, id: string) => {
		try {
			...
			await geetHospitalRefetch({ input: id });
			await getHospitalsRefetch({
				input: {
					page: 1,
					limit: 4,
					sort: 'createdAt',
					direction: Direction.DESC,
					search: {
						locationList: [hospital?.hospitalLocation],
					},
				},
			});
	...
	if (!router.isReady || !hospitalId || getHospitalLoading) {
		return (
		...
		return (
			<div id={'hospital-detail-page'}>
				<div className={'container'}>
					<Stack className={'hospital-detail-config'}>
						<Stack className={'hospital-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{hospital?.hospitalTitle}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{hospital?.hospitalLocation}</Typography>
										<Stack className={'divider'}></Stack>
										<Stack className={'buy-rent-box'}>
											{hospital?.hospitalBarter && (
												<>
										...
										<Typography className={'date'}>{moment().diff(hospital?.createdAt, 'days')} days ago</Typography>
									</Stack>
								...
								<Stack className={'right-box'}>
									<Stack className="buttons">
										<Stack className="button-box">
											...
											<Typography>{hospital?.hospitalViews}</Typography>
										</Stack>
										<Stack className="button-box">
							...
							<Stack className={'images'}>
								<Stack className={'main-image'}>
									<img
										src={slideImage ? `${REACT_APP_API_URL}/${slideImage}` : '/img/hospital/bigImage.png'}
										alt={'main-image'}
									/>
								</Stack>
								<Stack className={'sub-images'}>
									{hospital?.hospitalImages.map((subImg: string) => {
						...
						<Stack className={'hospital-desc-config'}>
								...
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{hospital?.hospitalBeds}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{hospital?.hospitalRooms}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{moment(hospital?.createdAt).format('YYYY')}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{hospital?.hospitalSquare} m2</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{hospital?.hospitalType}</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										<Typography className={'title'}>Hospital Description</Typography>
										<Typography className={'desc'}>{hospital?.hospitalDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>Hospital Details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												...
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Hospital Size</Typography>
													<Typography className={'data'}>{hospital?.hospitalSquare} m2</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Rooms</Typography>
													<Typography className={'data'}>{hospital?.hospitalRooms}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Bedrooms</Typography>
													<Typography className={'data'}>{hospital?.hospitalBeds}</Typography>
												</Box>
											</Stack>
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Year Built</Typography>
													<Typography className={'data'}>{moment(hospital?.createdAt).format('YYYY')}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Hospital Type</Typography>
													<Typography className={'data'}>{hospital?.hospitalType}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Hospital Options</Typography>
													<Typography className={'data'}>
														For {hospital?.hospitalBarter && 'Barter'} {hospital?.hospitalRent && 'Rent'}
													</Typography>
												</Box>
											</Stack>
										</Stack>
								...
								{commentTotal !== 0 && (
									<Stack className={'reviews-config'}>
										...
										<Stack className={'review-list'}>
											{hospitalComments?.map((comment: Comment) => {
								...
								<Stack className={'info-box'}>
									...
									<Stack className={'image-info'}>
										<img
											...
											src={
												hospital?.memberData?.memberImage
													? `${REACT_APP_API_URL}/${hospital?.memberData?.memberImage}`
													: '/img/profile/defaultUser.svg'
											}
										/>
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${hospital?.memberData?._id}`}>
												<Typography className={'name'}>{hospital?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{hospital?.memberData?.memberPhone}</Typography>
											</Stack>
...
export default withLayoutFull(HospitalDetail);
