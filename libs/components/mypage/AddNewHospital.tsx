...
import { CREATE_HOSPITAL, UPDATE_HOSPITAL } from '../../../apollo/user/mutation';
import { GET_HOSPITAL } from '../../../apollo/user/query';
import { sweetErrorHandling, sweetMixinErrorAlert, sweetMixinSuccessAlert } from '../../sweetAlert';

const AddHospital = ({ initialValues, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const inputRef = useRef<any>(null);
	const [insertHospitalData, setInsertHospitalData] = useState<HospitalInput>(initialValues);
	const [hospitalType, setHospitalType] = useState<HospitalType[]>(Object.values(HospitalType));
	const [hospitalLocation, setHospitalLocation] = useState<HospitalLocation[]>(Object.values(HospitalLocation));
	...
	/** LIFECYCLES **/
	useEffect(() => {
		setInsertHospitalData({
			...insertHospitalData,
			hospitalTitle: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalTitle : '',
			hospitalPrice: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalPrice : 0,
			hospitalType: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalType : '',
			hospitalLocation: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalLocation : '',
			hospitalAddress: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalAddress : '',
			hospitalBarter: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalBarter : false,
			hospitalRent: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalRent : false,
			hospitalRooms: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalRooms : 0,
			hospitalBeds: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalBeds : 0,
			hospitalSquare: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalSquare : 0,
			hospitalDesc: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalDesc : '',
			hospitalImages: getHospitalData?.getHospital ? getHospitalData?.getHospital?.hospitalImages : [],
		});
	}, [getHospitalLoading, getHospitalData]);
	...
	async function uploadImages() {
		try {
			...
			setInsertHospitalData({ ...insertHospitalData, hospitalImages: responseImages });
		} catch (err: any) {
	...
	const insertHospitalHandler = useCallback(async () => {
		try {
			const result = await createHospital({
				variables: {
					input: insertHospitalData,
				},
			});

			await sweetMixinSuccessAlert('This hospital has been created successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myHospitals',
				},
			});
		} catch (err: any) {
	...
	const updateHospitalHandler = useCallback(async () => {
		try {
			// @ts-ignore
			insertHospitalData._id = getHospitalData?.getHospital?._id;
			...
			await sweetMixinSuccessAlert('This hospital has been updated successfully.');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myHospitals',
	...
	console.log('+insertHospitalData', insertHospitalData);

	if (device === 'mobile') {
		return <div>ADD NEW HOSPITAL MOBILE PAGE</div>;
	} else {
		return (
			<div id="add-hospital-page">
				<Stack className="main-title-box">
					<Typography className="main-title">Add New Hospital</Typography>
					...
					<Stack className="config">
						<Stack className="description-box">
							<Stack className="config-column">
								...
								<input
									...
									placeholder={'Title'}
									value={insertHospitalData.hospitalTitle}
									onChange={({ target: { value } }) =>
										setInsertHospitalData({ ...insertHospitalData, hospitalTitle: value })
									}
								/>
							</Stack>
							...
							<Stack className="config-row">
								...
								<Stack className="price-year-after-price">
									...
									<select
										...
										defaultValue={insertHospitalData.hospitalType || 'select'}
										value={insertHospitalData.hospitalType || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertHospitalData({ ...insertHospitalData, hospitalType: value })
										}
									>
										<>
							...
							<Stack className="config-row">
								<Stack className="price-year-after-price">
									...
									<select
										...
										className={'select-description'}
										defaultValue={insertHospitalData.hospitalLocation || 'select'}
										value={insertHospitalData.hospitalLocation || 'select'}
										onChange={({ target: { value } }) =>
											// @ts-ignore
											setInsertHospitalData({ ...insertHospitalData, hospitalLocation: value })
										}
									>
										<>
											<option selected={true} disabled={true} value={'select'}>
											...
											{hospitalLocation.map((location: any) => (
								...
								<Stack className="price-year-after-price">
									...
									<input
										...
										className="description-input"
										placeholder={'Address'}
										value={insertHospitalData.hospitalAddress}
										onChange={({ target: { value } }) =>
							...
							<Typography className="hospital-title">Hospital Description</Typography>
							<Stack className="config-column">
								...
								<textarea
									...
									value={insertHospitalData.hospitalDesc}
									onChange={({ target: { value } }) =>
										setInsertHospitalData({ ...insertHospitalData, hospitalDesc: value })
									}
								></textarea>
							</Stack>
						</Stack>

						<Typography className="upload-title">Upload photos of your hospital</Typography>
						<Stack className="images-box">
							...
							<Stack className="gallery-box">
								{insertHospitalData?.hospitalImages.map((image: string) => {
						...
						<Stack className="buttons-row">
							{router.query.hospitalId ? (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={updateHospitalHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							) : (
								<Button className="next-button" disabled={doDisabledCheck()} onClick={insertHospitalHandler}>
									<Typography className="next-button-text">Save</Typography>
								</Button>
							)}
						</Stack>
