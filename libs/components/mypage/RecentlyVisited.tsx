...
const RecentlyVisited: NextPage = () => {
	...
	const [recentlyVisited, setRecentlyVisited] = useState<Hospital[]>([]);
	...
	if (device === 'mobile') {
		return <div>MEDI-CARE MY FAVORITES MOBILE</div>;
	} else {
		return (
			<div id="my-favorites-page">
				...
				<Stack className="favorites-list-box">
					{recentlyVisited?.length ? (
						recentlyVisited?.map((hospital: Hospital) => {
							return <HospitalCard hospital={hospital} recentlyVisited={true} />;
						})
					) : (
						<div className={'no-data'}>
							<img src="/img/icons/icoAlert.svg" alt="" />
							<p>No Recently Visited Hospitals found!</p>
						</div>
					)}
				</Stack>
				{recentlyVisited?.length ? (
					<Stack className="pagination-config">
						...
						<Stack className="total-result">
							<Typography>
								Total {total} recently visited hospital{total > 1 ? 's' : ''}
							</Typography>
						</Stack>
					</Stack>
				) : null}
