...
const MemberFollowings = (props: MemberFollowingsProps) => {
	...
	const category: any = router.query?.category ?? 'hospitals';
	...
	if (device === 'mobile') {
		return <div>MEDI-CARE FOLLOWS MOBILE</div>;
	} else {
		return (
							...
							<Stack className="follows-card-box" key={follower._id}>
								...
								{user?._id !== follower?.followingId && (
									<Stack className="action-box">
										{follower.meFollowed?.some(
											(follow) =>
												follow?.followingId === follower?.followingData?._id && follow?.followerId === user?._id,
										) ? (
											<>
